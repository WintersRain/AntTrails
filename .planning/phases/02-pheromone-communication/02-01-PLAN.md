...
NOTE — Gradient-following is already wired: Phase 1 (Plan 01-01) wired `foraging_movement()` from food.rs into the movement system. `foraging_movement()` already calls `follow_pheromone()` which calls `get_gradient()` on PheromoneGrid. Ants don't visibly follow trails today because pheromone saturation (everything at 1.0) makes `get_gradient()` return None. This plan fixes the math; Plan 02-01 Task 2 also replaces `get_gradient()` with `get_gradient_weighted()` inside `follow_pheromone()`, completing the upgrade. No new movement-system wiring is needed.
...
<task type="auto">
  ...
  **Update `follow_pheromone()` to use `get_gradient_weighted()` instead of `get_gradient()`:**
  ...
  NOTE: `follow_pheromone()` is already called by `foraging_movement()` in food.rs, which was wired into the movement system in Phase 1 (Plan 01-01). This edit upgrades the gradient method it uses -- no new call-site wiring is needed. Once pheromone values are correct (Task 1 fixes saturation), this existing call chain (`foraging_movement` -> `follow_pheromone` -> `get_gradient_weighted`) will cause ants to visibly turn toward stronger pheromone signals.
