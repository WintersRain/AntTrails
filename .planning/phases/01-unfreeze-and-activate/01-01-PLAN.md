...
<task type="auto">
  ...
  **In `src/systems/movement.rs`:**
  ...
  4. Replace the entire match block (lines 23-42) with explicit arms for ALL 8 AntState variants. The key changes:
     ...
     NOTE on AntState::Following: `foraging_movement()` does NOT check for specific AntState values when computing the food pheromone gradient -- it only checks state for Carrying vs Wandering return behavior. Following ants will fall through to the Wandering path (follow food pheromone), which is the desired behavior: Following ants track food pheromone trails, same as Wandering ants but without the random exploration fallback competing with trail-following.
